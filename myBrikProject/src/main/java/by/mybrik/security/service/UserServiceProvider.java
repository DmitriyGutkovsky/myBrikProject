...
//                        AuthorityUtils.commaSeparatedStringToAuthorityList(roleRepository
//                                .findUserRoles(user.getId()).stream()
//                                .map(Role::getRoleName).map(SystemRoles::name)
//                                .collect(Collectors.joining(",")))
                        AuthorityUtils.commaSeparatedStringToAuthorityList(usersRepository.getOne(user.getId()).getRoles().stream()
                        .map(Role::getRoleName).map(SystemRoles::name)
                        .collect(Collectors.joining(",")))
                );
            } else {
